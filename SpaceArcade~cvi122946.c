...
//--------------Panels 
static int panelHandle,panelHandleMain;

//--------------Global variables
int ship_img, backGround_img, MovingStars_img, won, loose ,extra_Ammo , extra_Life ;  	//Images
...
int LEVEL=1, laser_Factor=1, Mile_Stone=0, delay_bitween_attacks=500 , flag_extraLife=0,flag_extraAmmo=0;
...
int	Start_X_Location;
 ...
 //exra Life creation---------------------------------------------------	
  if(Player.LIFE<10 && flag_extraLife==0){
	flag_extraLife=1;
...
//ENEMIE SHOOT CREATION------------------------------------------------	enemie_Laser	
	...
	for( int i=0; i< active_small_Enemies ; i++){ 							//Take one enemie position each iteration
		if(Player_Ship.x + Player_Ship.width/2 == enemie[i].x+ enemie[i].width/2
		  											&& Player_Ship.y > enemie[i].y + enemie[i].hight) {  //check position of ship VS enemie  
			...
			enemie_Laser[active_Enemie_Shoots].vy=3 * LEVEL;
